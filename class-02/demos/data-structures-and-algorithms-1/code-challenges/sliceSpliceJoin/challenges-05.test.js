...
`);
// Got help from Ron Dunphy with this one
const templateWithJQuery = () => {
  starWarsPeople.forEach(obj =>
  {
    ...
    $section.find('h2').text(obj.name);
    $section.find('h3').text(obj.height);
    $section.find('p').text(obj.eye_color);
    $('main').append($section);
  });
...
// figured out how to locate the second white space here:
// https://stackoverflow.com/questions/36342430/get-substring-before-and-after-second-space-in-a-string-via-javascript

const listFoods = (recipe) => {
  let result = [];

  // recipe.ingredients.forEach( one =>
  //   {
  //     console.log('******+++++++++', one.slice(one.indexOf(' ', one.indexOf(' ') + 1));
  //     results.push(one.slice(one.indexOf(' ', one.indexOf(' ') + 1)));
  //   });
...
------------------------------------------------------------------------------------------------ */
// thinking use split with a white seperator to make array of every line item
// shift off the first 2 elements of new array
// join any reamaining elements
// push new string to results 
