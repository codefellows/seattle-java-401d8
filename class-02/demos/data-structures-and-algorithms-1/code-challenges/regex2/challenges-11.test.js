...
------------------------------------------------------------------------------------------------ */
const toLastNames = people => people.map(you => `${you.firstName} ${you.lastName}`)

// {
//   let names = people.map(you =>
//     {
//       let fullName = `${you.firstName} ${you.lastName}`
...
const validateEmail = (email) => /^(\w+\.+\w+|\w+)\@\w*\.\D{3}$/g.test(email)
...
const validatePhoneNumber = (phoneNumber) => /^(\(\d{3}\)|\d{3})[-\s\ ]?[-\s\ ]?\d{3}[-\s\ ]?\d{4}$/.test(phoneNumber)
